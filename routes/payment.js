...
router.post("/createPayment", async (req, res) => {
    if (!(await isOpen())) {
        res.json({
            customError:
                "We were unable to process your order because we are no longer taking orders at this time. We are open Monday to Friday 11-6.",
        });
        return;
    }
    ...
    async (request, response) => {
        ...
        switch (event.type) {
            case "payment_intent.succeeded":
                ...
                let order = await getOrderFromDatabase(
                    paymentIntent.metadata._id
                );
